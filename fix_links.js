...
const fileContent = fs.readFileSync('src/data/dsaTopics.js', 'utf8');

// We have an array inside. We can safely do a regex replace or just rewrite it.
// Actually, it's easier to just run the script that generated it again, but with better URL logic.
...
categories.forEach(cat => {
  cat.problems.forEach(prob => {
    // Generate leetcode search link since generating exact slugs is prone to error for non-standard titles
    // But if it's a known exact standard title, we can try to slugify safely.
    // Let's strip apostrophes first.
    let cleanProb = prob.toLowerCase().replace(/'/g, '');
    let slug = cleanProb.replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    let isGeneric = ['dfs-of-graph', 'bfs-of-graph', 'morris-inorder-traversal', 'morris-preorder-traversal', 'floor-in-bst', 'ceil-in-bst'].includes(slug);
    
    let url = isGeneric 
        ? `https://leetcode.com/problemset/all/?search=${encodeURIComponent(prob)}`
